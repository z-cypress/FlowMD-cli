 ...
 **单行优化已移除**：查询结果**始终以数组存储**，即使只返回一行也返回数组。类型可预测，模板写法不会随行数变化而变化。
 ...
 ```data {output: "user"}
 SELECT * FROM users WHERE id = 1   -- 单行结果 → user 仍是数组（长度为 1）
 ```
 ...
 {{user.0.name}}         -- 单行数组：用下标访问首行字段
 {{#with user.[0]}}      -- 单行数组：解构后直接访问字段
 {{name}} {{/with}}
 ```
